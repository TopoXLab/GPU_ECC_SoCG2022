...
std::vector<double> ECC_(std::string& fileName, int h, int w, int d, bool pad, bool async, bool mt, bool timing, bool verb) {
	...
	int chunk_num = (data_3D) ? imageD_ / 16 : imageH_ / 64;
	int engine_num = 3;
...
std::vector<double> ECC_folder(std::string& path, int h, int w, int d, bool pad, bool async, bool mt, bool timing, bool verb) {
	...
	const int pre_binNum = 1024;
	int chunk_num = 1000;// fileNames.size();
	int engine_num = 3;
	engine_num = (engine_num > chunk_num) ? chunk_num : engine_num;
...
void run_folder(std::string& path, int run_times) {
	...
	for (int i = 0; i < res.size(); i++) {
		...
		for (int j = 0; j < run_times; j++) {
			...
			if (dim == std::string("2"))
				time = ECC_(res[i], imageH_, imageW_, -1, pad_by_one, async_mode, mt_read, manual_timing, verbose);
			else
				time = ECC_(res[i], imageH_, imageW_, imageD_, pad_by_one, async_mode, mt_read, manual_timing, verbose);
...
void run_GENERAL(std::string& path, int data_dim, int run_times) {
	...
	for (int i = 0; i < res.size(); i++) {
		...
		for (int j = 0; j < run_times; j++) {
			std::vector<double> time;
			time = (data_dim == 2) ? ECC_(res[i], imageH_, imageW_, -1, pad_by_one, async_mode, mt_read, manual_timing, verbose) :
				ECC_(res[i], imageH_, imageW_, imageD_, pad_by_one, async_mode, mt_read, manual_timing, verbose);
